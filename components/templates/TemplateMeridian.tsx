...
export default function TemplateMeridian({ portfolioData }: { portfolioData: PortfolioData }) {
  const { personal, experience, projects, skills, education, certifications, tagline, careerStory, workStyle, lookingFor, languages = [], interests = [] } = portfolioData
  ...
  return (
    <>
      <style>{`
        ...
        /* LANGUAGES */
        .tm-lang-list { display: flex; flex-direction: column; gap: 18px; max-width: 480px; }
        .tm-lang-item { display: flex; flex-direction: column; gap: 8px;
          opacity: 0; transform: translateX(-16px);
          transition: opacity 0.6s var(--tm-ease-out), transform 0.6s var(--tm-spring);
        }
        .tm-lang-item.tm-revealed { opacity: 1; transform: translateX(0); }
        .tm-lang-row { display: flex; justify-content: space-between; align-items: baseline; }
        .tm-lang-name { font-family: var(--tm-font-display); font-size: 16px; font-weight: 700; }
        .tm-lang-level { font-family: var(--tm-font-mono); font-size: 11px; color: var(--tm-accent); letter-spacing: 0.1em; }
        .tm-lang-track { height: 2px; background: var(--tm-border); border-radius: 1px; overflow: hidden; }
        .tm-lang-fill { height: 100%; background: var(--tm-accent); border-radius: 1px; width: 0%; transition: width 1s var(--tm-ease-out) 0.3s; }
        .tm-lang-item.tm-revealed .tm-lang-fill { width: var(--fill); }

        /* INTERESTS */
        .tm-interests-wrap { display: flex; flex-wrap: wrap; gap: 10px; }
        .tm-interest-chip {
          font-family: var(--tm-font-mono); font-size: 11px; letter-spacing: 0.06em;
          color: var(--tm-primary); background: var(--tm-accent-light);
          border: 1px solid rgba(224,122,95,0.25); border-radius: 20px;
          padding: 6px 16px;
          opacity: 0; transform: scale(0.88);
          transition: opacity 0.4s var(--tm-spring), transform 0.4s var(--tm-spring), background 0.2s;
        }
        .tm-interest-chip.tm-popped { opacity: 1; transform: scale(1); }
        .tm-interest-chip:hover { background: var(--tm-accent); color: #fff; }
      ...
      <div className="tm-root">
        ...
        <div className="tm-layout">
          ...
          <main className="tm-main">
            ...
            {/* LANGUAGES */}
            {languages.length > 0 && (
              <section id="tm-languages" className="tm-section">
                <div className="tm-section-label tm-reveal">Languages</div>
                <h2 className="tm-section-heading tm-reveal tm-reveal-d1">Communication</h2>
                <div className="tm-lang-list">
                  {languages.map((l: any, i: number) => {
                    const lvl = (l.proficiency || l.level || "").toLowerCase()
                    const fill = lvl.includes("native") || lvl.includes("fluent") ? "100%" : lvl.includes("professional") || lvl.includes("advanced") ? "80%" : lvl.includes("intermediate") ? "60%" : "40%"
                    return (
                      <div key={i} className="tm-lang-item" style={{ transitionDelay: `${i * 0.1}s` }}>
                        <div className="tm-lang-row">
                          <span className="tm-lang-name">{l.language || l.name}</span>
                          <span className="tm-lang-level">{l.proficiency || l.level || "Fluent"}</span>
                        </div>
                        <div className="tm-lang-track">
                          <div className="tm-lang-fill" style={{ "--fill": fill } as React.CSSProperties} />
                        </div>
                      </div>
                    )
                  })}
            ...
            {/* INTERESTS */}
            {interests.length > 0 && (
              <section id="tm-interests" className="tm-section">
                <div className="tm-section-label tm-reveal">Personal</div>
                <h2 className="tm-section-heading tm-reveal tm-reveal-d1">Interests</h2>
                <div className="tm-interests-wrap">
                  {(interests as any[]).map((item: any, i: number) => (
                    <span key={i} className="tm-interest-chip" style={{ transitionDelay: `${i * 0.07}s` }}>
...
function TMEffects({
  ...
  useEffect(() => {
    ...
    const langObs = new IntersectionObserver(
      (entries) => entries.forEach((e) => {
        if (e.isIntersecting) { e.target.classList.add("tm-revealed"); langObs.unobserve(e.target) }
      }),
      { threshold: 0.2 }
    )
    document.querySelectorAll(".tm-lang-item").forEach((el, i) => {
    ...
    const intObs = new IntersectionObserver(
      (entries) => entries.forEach((e) => {
        if (e.isIntersecting) {
          e.target.querySelectorAll(".tm-interest-chip").forEach((chip, ci) => {
            setTimeout(() => chip.classList.add("tm-popped"), ci * 70)
          })
    ...
    document.querySelectorAll("#tm-interests").forEach((el) => intObs.observe(el))
